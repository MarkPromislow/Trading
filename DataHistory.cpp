...
inline
DataBucket* BucketHistory::_bucketForOffset(unsigned offset)
{
...
void BucketHistory::addAggregator(Aggregator * aggregator)
{
...
int BucketHistory::addTime(uint64_t currentTime)
{
	if (currentTime > _previousTime)
	{
		...
		if (_previousData)
		{
			...
			for (Intrusive::LinkedListObject *obj = _realTimeAggregators.begin(); obj != _realTimeAggregators.end(); obj = obj->next())
			{
				static_cast<Aggregator*>(obj)->addTime(duration, _previousData);
			}
		}
...
void TimedDataAggregator::addTime(uint64_t currentTime)
{
	...
	for (; _newest->next() != _dataQueue->end();)
	{
		...
		{
			_dataBucket->addTime(data->time() - _newestTime, _newest);
			_newestTime = data->time();
		}
		_dataBucket->addData(data);
		_newest = data;
	...
	{
		_dataBucket->addTime(startTime - _newestTime, _newest);
	...
	for (; _oldest->next() != _dataQueue->end();)
	{
		...
		{
			_dataBucket->subTime(data->time() - _oldestTime, _oldest);
			_oldestTime = data->time();
		}
		_dataBucket->subData(data);
		_oldest = data;
	...
	{
		_dataBucket->subTime(endTime - _oldestTime, _oldest);
		_oldestTime = endTime;
	}
}
